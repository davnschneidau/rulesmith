    ...
    def before_node(self, node_name: str, state: Dict[str, Any], context: Any) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                log_error(
                    logger,
                    f"Error in before_node hook for '{node_name}'",
                    e,
                    context={"node_name": node_name, "hook": hook.__class__.__name__},
                )
                # Don't fail execution on hook errors
    ...
    def after_node(self, node_name: str, state: Dict[str, Any], context: Any, outputs: Dict[str, Any]) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                log_error(
                    logger,
                    f"Error in after_node hook for '{node_name}'",
                    e,
                    context={"node_name": node_name, "hook": hook.__class__.__name__},
                )
    ...
    def on_error(self, node_name: str, state: Dict[str, Any], context: Any, error: Exception) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                logger.warning(
                    f"Error in on_error hook for '{node_name}': {type(e).__name__}: {str(e)}",
                    exc_info=True,
                )
    ...
    def on_guard(self, node_name: str, state: Dict[str, Any], context: Any, guard_result: Any) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                log_error(
                    logger,
                    f"Error in on_guard hook for '{node_name}'",
                    e,
                    context={"node_name": node_name, "hook": hook.__class__.__name__},
                )
    ...
    def on_hitl(self, node_name: str, state: Dict[str, Any], context: Any, request_id: str) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                log_error(
                    logger,
                    f"Error in on_hitl hook for '{node_name}'",
                    e,
                    context={"node_name": node_name, "request_id": request_id, "hook": hook.__class__.__name__},
                )
